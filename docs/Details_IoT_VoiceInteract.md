...
*(已移除尋址燈條引腳。由於實體燈條應與櫃位 RFID 讀取器就近安裝，為簡化實體接線並採模組化「一層一機」設計，WS2812B 尋址燈條已遷移至 **節點二 (ESP32 #2) 邊緣庫存狀態感測站** 控制)*
...
### 狀態變數與暫態定義
* `STATE_IDLE`：本地狀態指示燈（或呼吸燈）緩慢閃爍，等待使用者按下 PTT。
...
* `STATE_PLAYING`：本地指示燈呈現播音狀態，同時解碼播放 BFF 回傳的 Base64 語音數據。（註：櫃位實體尋址燈條由對應之節點二在接收到系統指令後同步點亮）。
...
## 6. 本地狀態指示燈與互動視覺實作 (Status LED & Local Visuals)

> [!NOTE]
> **視覺引導燈條架構變更說明：**
> 為了避免從中央語音站拉長距離訊號線到各個櫃架，我們將 WS2812B 櫃位視覺指引燈條改由 **節點二 (ESP32 #2 - 邊緣庫存狀態感測站)** 來驅動。
> 節點一 (ESP32 #1) 此處僅需負責 **本地狀態指示燈**（如 PTT 按鍵周圍的環狀 LED，或 GPIO 2 上的本地小 LED）之微互動特效，以提供使用者直覺的操作反饋。

### 6.1 💡 麥克風站本地微互動特效 (WOW Effects)
為了營造精緻的科技感，語音站在進行不同交互時，其本地狀態指示燈（如一個 WS2812B 單燈或環形燈圈）應實作以下微動畫：

1. **待機狀態 (STATE_IDLE)：** 呈現微弱的暖白色或藍色呼吸效果，暗示設備已就緒且正在安靜監聽。
2. **錄音中狀態 (STATE_RECORDING)：** 按下 PTT 時，本地狀態燈呈現溫暖紅光緩慢淡入淡出（呼吸特效），警告使用者麥克風已開啟。
3. **雲端運算中 (STATE_SENDING)：** 音訊上傳至 BFF 期間，本地狀態燈以藍色或青色進行快速跑馬燈或旋轉流光，暗示系統正在「思考計算中」。
4. **定位/語音播報中 (STATE_PLAYING)：** 播放語音時，本地狀態燈以綠色呼吸流光閃動，指引使用者注意播音與櫃位燈條。
5. **錯誤狀態 (STATE_ERROR)：** 網路連線失敗或找不到器材時，本地狀態燈以紅光快速閃爍 3 次，並配合小喇叭發出「嗶嗶嗶」警告音。
...
- [ ] **6. 本地狀態 LED 指示與互動整合**
  - [ ] 實作本地狀態指示燈呼吸與思考中旋轉流光效果
  - [ ] 與語音播放/錄音狀態同步，完成人機交互介面視覺優化
- [ ] **7. 整體聯調測試與優化**
